...
vector<double> UncalDate::compute_probs(vector<int> error_cal_curve, vector<int> full_c14_bp){
	...
	while (t < error_cal_curve.size()){
	   double this_tau = pow(_std,2) + error_cal_curve[t];
...
void UncalDate::generate_date_grid(std::vector<int>& full_bp, std::vector<int>& full_c14_bp, std::vector<int>& full_error, CalCurve& calcurve){
	...
	while (i < num_elements) {
		int this_bp = calcurve.max_bp_cal_curve() - i * 5;
    ...
    if(it!=cal_bp.end()){
	    ...
	    this_error = error[pos];
		    ...
		    vector<int> working_error = error;
				...
				double mu = (float)(this_bp-lower_bp) / (float)(upper_bp - lower_bp);
				this_c14 =  (int) round(LinearInterpolate((double)lower_c14, (double)upper_c14,mu));
				this_error =  (int) round(LinearInterpolate((double)lower_error, (double)upper_error,mu));
    }
	full_bp[i] = this_bp;
...
double UncalDate::LinearInterpolate(double y1, double y2, double mu){
